create or replace package body AP_VALID_PKG_10 as
  ...
  Cursor cur_AP_order_header
     IS
            SELECT          XAPIS.vendor_number
                          , XAPIS.vendor_id
                          , XAPIS.document_type_code
                          , XAPIS.agent_number
                          , XAPIS.ship_to_location
                          , XAPIS.ship_to_location_id
                          , XAPIS.bill_to_location
                          , XAPIS.bill_to_location_id
                          , XAPIS.curr_code
                    FROM  PURCHASE_ORDER10_STG  XAPIS
                    WHERE UPPER(XAPIS.record_status) = gc_new_status
                    GROUP BY XAPIS.vendor_number
                           , XAPIS.vendor_id
                           , XAPIS.document_type_code
                           , XAPIS.agent_number
                           , XAPIS.ship_to_location
   ...
   Cursor  cur_AP_order_lines (
     ...
     IS
       SELECT XAPIS.*
       FROM   PURCHASE_ORDER10_STG  XAPIS
       WHERE  XAPIS.vendor_number          = p_vendor_number
       AND    XAPIS.ship_to_location       = p_ship_to_location
       AND    XAPIS.bill_to_location       = p_bill_to_location;
   ...
   begin
        ...
        mo_global.set_APlicy_context('S',FND_PROFILE.VALUE('USER_ID')); 
        ...
        for   i in cur_AP_order_header loop
              counter := 0;
              --Get Interface Header ID from sequence
              select AP_HEADERS_INTERFACE_S.nextval
                into ln_AP_header_id
                from dual;             
              --Lets Validate
              --1. Currency Code
               FND_FILE.PUT_LINE(FND_FILE.output,'Header Id : '||ln_AP_header_id);
              
             begin
               AP_VALID_PKG_10.Validate_Currency_10(i.curr_code,l_curr_code );
             ...
             l_vendor_id := AP_VALID_PKG_10.get_VendorID_10(i.vendor_number);
             ...
             l_agent_id := AP_VALID_PKG_10.get_EmployeeId_10(i.agent_number);
...
fnd_file.put_line(fnd_file.log,'Error Flag is :-'||l_error_flag);
      ...
      INSERT INTO AP_headers_interface (
                                          interface_header_id
                                        , batch_id
                                        , action
                                        , document_type_code
                                 ...
                                 VALUES ( ln_AP_header_id
                                        ...
                                        , l_agent_id
                                        , l_vendor_id                                       
                                        , AP_VALID_PKG_10.get_vendor_siteId_10(l_vendor_id) 
                                       -- ,i.bill_to_location
                                        , 'INCOMPLETE'
                                        , i.ship_to_location
                                        , i.Ship_to_location
...
FOR j IN cur_AP_order_lines (
       ...
       LOOP
         SELECT AP_LINES_INTERFACE_S.nextval
         INTO ln_AP_line_id
         FROM DUAL;
         
         -- Validating Item Number 
       dbms_output.put_line(ln_AP_line_id);
       BEGIN
            
               l_inventory_item_id :=  AP_VALID_PKG_10.get_inventory_itemId_10(j.inventory_item );
...
dbms_output.put_line (ln_AP_header_id||','|| ln_AP_line_id||','||l_inventory_item_id||','||j.unit_of_measure||','|| j.unit_price||','||j.quantity||','||j.line_number||','||gn_user_id
                                        );
                                 counter := counter + 1;      
insert into AP_lines_interface (
                                          interface_header_id
                                        , interface_line_id
                                        , line_type
                                        , item_id
                                 ...
                                 VALUES (
                                        ...
                                        , l_inventory_item_id
                                        , j.inventory_item
                                        , AP_VALID_PKG_10.get_inventory_itemDscr_10(j.inventory_item)
                                        , j.unit_of_measure
                                        , j.unit_price
                                        , j.quantity
                                        , TRUNC(SYSDATE) + 1
  ...
  commit;    
   ...
   procedure Validate_Currency_10(p_curr_code fnd_currencies.CURRENCY_CODE%Type,P_ret_txt OUT Number )
  is
    ...
    exception when too_many_rows then
              ...
              when others then
              P_ret_txt := 2;
    end Validate_Currency_10;
    --
    function get_VendorID_10(p_vendor_num ap_suppliers.segment1%type )
    return number is
 ...
 function get_EmployeeId_10(p_agent_number per_all_people_f.employee_number%type) 
    return number is
    ...
    exception when no_data_found then
              ...
              when others then
              return(0);
    end get_EmployeeId_10;  
function get_inventory_itemId_10(p_inv_item mtl_system_items.segment1%type )
                           return number
is
...
exception when no_data_found then
          ...
          when others then
          return(99999);
end get_inventory_itemId_10;

function get_vendor_siteId_10(p_vendor_id ap_suppliers.vendor_id%type) 
return varchar2 is 
...
end get_vendor_siteId_10 ;


function get_inventory_itemDscr_10(p_inv_item mtl_system_items.segment1%type )
                           return varchar2
is
